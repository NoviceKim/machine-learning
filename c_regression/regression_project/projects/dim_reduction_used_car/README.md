...
결측치가 10% 이상인 컬럼들 중, 데이터 타입이 문자형인 feature, Drive Type과 Fuel Suppy System은  
이번 프로젝트에선 결측치를 제거하기로 결정하여, 위와 마찬가지로 결측치를 제거함

그 후, 결측치 10% 이상인 수치형 컬럼들을 describe()로 평균과 중간값을 확인해 본 결과,  
...
- 이 과정 이후, 차원(feature) 수가 7 이상이 되면 PCA 방식으로 차원 축소 실행
